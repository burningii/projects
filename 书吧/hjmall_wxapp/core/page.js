module.exports = {
    ...
    unionLogin: function(e) {
        let user_obj = e
        wx.setStorageSync('user_obj', user_obj);
        ...
        getApp().core.showLoading({
            ...
            success: function(e) {
                if (0 == e.code) {
                    ...
                    // n.setPhone())
                    n.setPhone()
                    n.setUserInfoShowFalse();
                    if (!e.data.binding){
                        n.pageToVercify()
                    }else{
                        let userinfoFlag = wx.getStorageSync('showuserInfoFlag');
                        if (userinfoFlag && userinfoFlag==1){
                            // wx.redirectTo({
                            //     url: `/pages/selectLocation/index?storeid=${show_id_my}`
                            // })
                            wx.navigateBack()
                        }else{
                            wx.reLaunch({
    ...
    // 跳转验证码
    pageToVercify(){
        let shop_id= wx.getStorageSync('shop_id_my');
        wx.navigateTo({
            url: `/pages/verficity/index?storeid=${shop_id}`
        })
        // wx.reLaunch({
        //     url: `/pages/verficity/index?storeid=${shop_id}`
        // })
