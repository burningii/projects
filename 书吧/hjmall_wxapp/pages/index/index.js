...
Page({
    ...
    loadData: function() {
        ...
        console.log('user_info_show数据为: ')
        console.log(i.data.user_info_show)
        getApp().request({
            url: getApp().api.default.index,
            data: t,
            success: function(t) {
                let shop_list=null
                t.data.template.forEach(item=>{
                    if (item.type=='mch')shop_list=item.param.list
                })
                wx.setStorageSync('shop_list', shop_list);
                if (0 == t.code) {
    ...
    onShow: function() {
        ...
        console.log('开始了哦123: ');
        console.log(e.data.user_info_show)
    },
